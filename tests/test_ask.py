...
def test_ask_refuses_without_calling_the_llm_when_nothing_retrieved_at_all(monkeypatch):
    ...
    assert result == {"answer": ask_module.NO_ANSWER, "citations": [], "usage": {"prompt_tokens": 0, "completion_tokens": 0}}
...
def test_ask_returns_answer_with_deduped_citations(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda q, chunks, **kw: {"answer": "The answer is X [1][2].", "citations": [_CHUNK_A, _CHUNK_B], "usage": _USAGE},
    )
...
def test_ask_dedupes_citations_from_the_same_article(monkeypatch):
    ...
    monkeypatch.setattr(
        ask_module, "generate_answer",
        lambda q, chunks, **kw: {"answer": "X [1][2].", "citations": [_CHUNK_A, chunk_a2], "usage": _USAGE},
    )
...
def test_ask_treats_llm_refusal_as_no_citations_even_with_retrieved_chunks(monkeypatch):
    ...
    monkeypatch.setattr(
        ask_module, "generate_answer",
        lambda q, chunks, **kw: {"answer": "I don't know.", "citations": [_CHUNK_A], "usage": _USAGE},
    )

    result = ask_module.ask("a question the chunks don't actually answer")

    assert result == {"answer": "I don't know.", "citations": [], "usage": _USAGE}
...
def test_ask_recognizes_refusal_variants_case_and_punctuation_insensitively(monkeypatch):
    ...
    for variant in ["I don't know", "i don't know.", "I DO NOT KNOW."]:
        monkeypatch.setattr(
            ask_module, "generate_answer",
            lambda q, chunks, variant=variant, **kw: {"answer": variant, "citations": [_CHUNK_A], "usage": _USAGE},
        )
...
def test_ask_falls_back_to_related_tier_when_nothing_is_accepted(monkeypatch):
    ...
    def fake_generate(q, chunks, **kw):
        ...
        return {
            "answer": "The sources don't directly cover this, but generally...",
            "citations": [_CHUNK_RELATED],
            "usage": _USAGE,
        }
...
def test_ask_related_tier_still_refuses_if_the_llm_finds_nothing_useful(monkeypatch):
    ...
    monkeypatch.setattr(
        ask_module, "generate_answer",
        lambda q, chunks, **kw: {"answer": "I don't know.", "citations": [_CHUNK_RELATED], "usage": _USAGE},
    )
...
def test_ask_prefers_accepted_over_related_when_both_are_present(monkeypatch):
    ...
    def fake_generate(q, chunks, **kw):
        captured["chunks"] = chunks
        captured["related_kwarg"] = kw.get("related")
        return {"answer": "Direct answer [1].", "citations": [_CHUNK_A], "usage": _USAGE}
...
def test_ask_no_retrieval_at_all_reports_zero_usage(monkeypatch):
    # The fast-refusal path never calls the LLM -- its usage must be a
    # real zero, not missing or None, so eval.py can sum it unconditionally.
    monkeypatch.setattr(ask_module, "retrieve", lambda q: _retrieval())
    monkeypatch.setattr(ask_module, "generate_answer", lambda q, chunks, **kw: (_ for _ in ()).throw(AssertionError))
